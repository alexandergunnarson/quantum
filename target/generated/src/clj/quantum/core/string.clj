(ns quantum.core.string
  ...
  (:require
    [quantum.core.function :as fn :refer
             [compr f*n fn* unary fn->> fn-> <- defprotocol+]
                                  
                          
                            ]
    [quantum.core.logic :as log :refer
             [splice-or fn-and fn-or fn-not ifn if*n whenc whenf whenf*n whencf*n condf condf*n condfc nnil?]
    ...
    [quantum.core.reducers :as r :refer
             [map+ reduce+ filter+ remove+ take+ take-while+ reducei+ fold+ range+ for+]
...
(defn lower-case? [c]
         (Character/isLowerCase ^Character c)
                               )
(defn char+ [obj]
...
(defn starts-with?
  ...
  (cond
    (string? super)
             (.startsWith ^String super ^String sub)
...
(defn ends-with?
  ...
  (cond
    (string? super)
             (.endsWith ^String super ^String sub)
