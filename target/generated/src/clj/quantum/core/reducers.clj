(ns ^{:doc
  ...
  (:require
    [quantum.core.function :as fn :refer
             [compr f*n fn* unary monoid firsta call rfn fn->> fn-> <- defcurried]
    ...
    [quantum.core.type     :as type :refer
      [      bigint?              instance+? array-list? boolean? double? map-entry?
...
(defn reduce+
  ...
  ([f init coll]
    (if        (instance? java.util.Map coll)
        ...
        (do 
            (try (       clojure.core.protocols/kv-reduce
...
(def fold-size
  ...
  (fn-> count dec  
       ...
       ) ; Because it's only single-threaded anyway...
...
(defn fold+
  ...
  ([obj]
          (whenf (fold-pre obj) (fn-or reducer? lseq?) vec+)
    (->> obj fold-pre (into+ [])))
  ...
  CollFold
    (coll-fold
     [      _             n combinef reducef]
                                   ; For ClojureScript, |fold| just falls back on reduce. No crazy async things.
...
(defn append!
  ...
  {:added "1.5"
   :attribution "clojure.core.reducers"}
  [acc x]
         (doto ^java.util.Collection acc (.add  x))
                             )
