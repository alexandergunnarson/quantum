(ns ^{:doc
  ...
  quantum.core.reducers
         (:refer-clojure :exclude [Range ->Range])
  (:require
    [quantum.core.function :as fn :refer
                                                                                  
             [compr f*n fn* unary monoid firsta call]
             :refer-macros
    ...
    (if                                      
        ...
        (cond
          ...
          (array? coll) (array-reduce coll f init)
                 (set?    coll)        (reduce-seq coll f init)
...
(def reducer? 
  (compr class
    (fn-or                                                          
                  (f*n instance+? cljs.core/IReduce)
  ...
  CollFold
    (coll-fold
     [               this n combinef reducef]
            (-reduce this reducef) ; For ClojureScript, |fold| just falls back on reduce. No crazy async things.
...
(defn distinct-by+ ; 228.936664 ms (pretty much attains java speeds!!!)
  ...
  ^{:attribution "parkour.reducers"}
  [f coll]
  (let [sentinel                        (array)] ; instead of nil, because it's unique
...
;;;;;;;;;;;; This file autogenerated from src/cljx/quantum/core/reducers.cljx
