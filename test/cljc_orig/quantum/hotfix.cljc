...
; FIXES "cond requires an even number of forms"

;; Core API for reactive binding
#?(:cljs (deftype BindingInfo [raw-deref add-watch remove-watch clean]))
...
#?(:cljs (def ^:private auto-reactive-id 0))
...
#?(:clj
   (def ^:private cursor-binding-info
     (freactive.IReactive$BindingInfo.
      ...
      (fn [cursor k f]
        (.addWatch ^clojure.lang.IRef cursor k f))
      (fn [^clojure.lang.IRef cursor k] (.removeWatch cursor k))
...
(deftype Cursor
  ...
  (notifyWatches [this old-state new-state]
    ...
    (doseq [[key f] #?(:clj @watches        :cljs watches)]
      (try 
        ; https://github.com/aaronc/fx-clj/blob/29639356d8d1253438ecf61e123caacefa9269ec/src/fx_clj/binding/RefObservable.java
        ; CHANGE LISTENER
        (f key this old-state new-state)
  ...
  (-add-change-watch [this key f]
    #?(:clj
       (when-not (contains? @change-watches key)
         (clojure.core/swap! change-watches assoc key f)
         ; TODO IMPLEMENT EQUIVALENT OF (.registerOne this)
         ))
    #?(:cljs
       (when-not (contains? change-watches key)
  ...
  (-remove-change-watch [this key]
    #?(:clj
       (when (contains? @change-watches key)
         (clojure.core/swap! change-watches dissoc key)
          ; TODO IMPLEMENT EQUIVALENT OF (.unregisterOne this)
          ))
    #?(:cljs
       (when (contains? change-watches key)
  ...
  #?@(:cljs
       ...
       [IRef
        ...
        (addWatch [this key f]
                   (clojure.core/swap! watches assoc key f)
                  this)
  ...
  #?@(:cljs
       ...
       [IReference
        (alterMeta [this f args]
                   (clojure.core/swap! meta f args))
        (resetMeta [this new-val]
                   (clojure.core/reset! meta new-val))]))
...
(defn atom
  ...
  #?(:cljs
     (Cursor.
      ...
      validator) 
     :clj
     (let [state-ref (clojure.core/ref init)
           notify-agent (clojure.core/agent nil)]
       (Cursor. nil
                nil
                (fn [] @state-ref)
...
#?(:cljs (def ^:dynamic *do-trace-captures* nil))
#?(:cljs (def ^:dynamic *trace-capture*     nil))
#?(:cljs
