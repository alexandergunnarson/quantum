...
(defn -main [& args]
  (println "This is the main method output."))
...
;CompilerException java.lang.StringIndexOutOfBoundsException: String index out of range: 3
...
;========================================================={ OTHER HELPERS }=========================================================
;========================================================={               }=========================================================
;(defmacro get-fx "fetches a property from a node." [obj prop & args]
...
;; TODO inefficient.
(defn get-fx "fetches a property from a node." [obj prop & args]
...
;; TODO inefficient.
(defn set-fx! "creates and applies a setter." [obj prop & args]
  (let [method (if (= "?" (subs (name prop) (dec (count (name prop)))))
                 (symbol (str (prepend-and-camel "set" (subs (name prop) 0 (dec (count (name prop)))))))
                 (symbol (str (prepend-and-camel "set" (name prop)))))]
    (apply exec-method obj method args)))

;(defmacro set-fx! [obj & args]
;  (let [m# (apply hash-map args)] ; args become a hash-map
;    `(do ~@(for [entry# m#] ; entries in map
;             `(core/bind-property!* ; bind-property to object
;                ~obj ~(key entry#)
;                (atom ~(val entry#))))))) 
;___________________________________________________________________________________________________________________________________
;======================================================{ COLLECTION HELPERS }=======================================================
...
(defmacro def-sym-fx [sym ctrl & props]
  `(intern *ns* ~sym (fx ~ctrl ~@props)))

(defmacro get-prop-fx "fetches a property from a node." [obj prop & args]
  (if (= \? (subs (name prop) (dec (count (name prop)))))
      "Error!"
