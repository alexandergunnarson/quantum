...
(defn java-file-watcher
  ...
  (async thread-opts
    (with-resources
      ...
      (let [^java.nio.file.Path path
              (doto (convert/->path [:resources])
                (.register watch-service
                  (into-array event-kinds)
                  (into-array event-modifiers)))]
        (while (empty? close-reqs)
          (let [^WatchKey watch-key (.take watch-service)] ; Blocks until whatsit
            (when watch-key
        ...
        (async-loop thread-opts
          ...
          (let [^File file-n  (-> @file convert/->file)
                _ (with-throw (io/file? file-n)
                    "File to watch is no longer a java.io.File.")
                _ (with-throw (io/exists? file-n)
                    (throw+ (Err. :early-termination "File deleted. Watch over." file-n)))
                file-changed? (not= (.lastModified file-n) prev-timestamp)]
            (if file-changed?
                (do (try+ (modified file-n)
                      (catch Object e
...
; (defmacro ^:private predicate {:source "me.raynes/fs"} [s path]
;   `(if ~path
;      (. ~path ~s)
;      false))

; (defn absolute?   [path] (predicate isAbsolute (io/file path)))
; (defn executable? [path] (predicate canExecute (file    path)))
; (defn readable?   [path] (predicate canRead    (file    path)))
; (defn writeable?  [path] (predicate canWrite   (file    path)))
; (defn delete      [path] (predicate delete     (file    path)))
; (defn exists?     [path] (predicate exists     (file    path)))
; (defn absolute    [path] (.getAbsoluteFile     (file    path)))
; (defn normalized  [path] (.getCanonicalFile    (file    path)))
...
; (defn directory? [path] (predicate isDirectory (file path)))

; (defn file? [path] (predicate isFile (file path)))

; (defn ^Boolean hidden? [path] (predicate isHidden (file path)))
...
; (defn extension [path] (last (split-ext path)))
; (defn name      [path] (first (split-ext path)))
; (defn parent    [path] (.getParentFile (file path)))
; (defn mod-time  [path] (.lastModified (file path)))
; (defn size      [path] (.length (file path)))
; (defn mkdir     [path] (.mkdir (file path)))
; (defn mkdirs    [path] (.mkdirs (file path)))
...
; (defn create [^File f] (.createNewFile f))
...
; (defn- f-dir?     [^File f] (when f (.isDirectory f)))

; (defn- f-children [^File f] (.listFiles f))
