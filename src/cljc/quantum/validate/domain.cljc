...
; TODO import unit tests from org.apache.commons.validator.routines.DomainValidator

; Regexes for hostnames (derived from RFC2396 and RFC 1123)
; RFC2396: domainlabel   = alphanum | alphanum *( alphanum | "-" ) alphanum
; Max 63 characters
(def domain-label-regex "\\p{Alnum}(?>[\\p{Alnum}-]{0,61}\\p{Alnum})?")

; RFC2396 toplabel = alpha | alpha *( alphanum | "-" ) alphanum
; Max 63 characters
(def top-label-regex    "\\p{Alpha}(?>[\\p{Alnum}-]{0,61}\\p{Alnum})?")
...
; If the match fails, input is checked against domain-label-regex (hostname-regex)
...
(defn valid?
  "Returns true if the @domain-0 parses
   ...
   This class does not provide IP address lookup for domain names or
   methods to ensure that a given domain name matches a specific IP;
   see |java.net.InetAddress| for that functionality."
