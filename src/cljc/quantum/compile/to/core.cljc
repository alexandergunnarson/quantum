...
(def reserved-syms
  {:js   #{'try 'catch 'finally 'arguments 'this 'for 'if 'else 'class 'function}
...
(def special-syms
  (atom
    {'for
       ...
       (fn def*-fn [[special-sym global? sym form & args]]
         ...
         (let [_ (log/pr :debug "META FOR SYM" sym "IS" (meta sym))
               ...
               form-f
                 (condp = *lang* 
                   :js (if class-
                           (do (log/pr :debug "CREATING VAR WITH CLASS" class-)
     ...
     'when
       (fn when-fn [[special-sym pred expr-t & args]]
         {:pre [(with-throw (-> args count (= 0))
                  (str/sp "|when| takes two arguments. Supplied extra:" args))]}
         (let [standard #(let [pred-str (str/sp "if" (str/paren (eval-form pred)))
       ...
       (fn fn*-fn [[spec-sym lambda? unk & body]]
         (let [sym (when (symbol? unk) unk)
               ...
               fn-arglist
                 (->> arglist-f
                      (map (fn [sym]
                             (str/sp (or (metaclass sym) "Object")
                                     (eval-form sym))))
                      (str/join ", ")
                      str/paren)
               fn-header
