...
(t/defn ^:inline educe
  ...
  ([rf rf?,              xs (t/input reduce [:_ :_ :?])] (educe rf (rf) xs))
  ([rf rf?, init t/any?, x  dasync/read-chan?] (async/go (rf (async/<! (reduce rf init x)))))
  ([rf rf?, init t/any?, xs (t/input reduce [:_ :_ :?])] (rf (reduce rf init xs))))
...
#?(:clj  ([xs dc/java-map?     > p/int?] (.size xs)))
         ;; Not counted
         ([xs (t/input educe [:_ :_ :?])] (educe count|rf xs)))
...
(t/defn ^:inline bounded-count > dn/std-integer?
  ...
  ([n dn/std-integer?, xs (t/input educe [:_ :_ :?])] (educe (gen-bounded-count|rf n) xs)))
