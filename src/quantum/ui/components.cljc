...
#?(:cljs (def image               (rx-adapt react-native "Image")))



; var CacheImage = require('@remobile/react-native-cache-image'); doesn't work on web ; better to have something else
#?(:cljs (def touchable-highlight (rx-adapt react-native "TouchableHighlight")))
#?(:cljs (def accordion           (when-not (= sys/os "web")
                                    (err/ignore (rx/adapt-react-class (js/require "react-native-accordion"))))))
#?(:cljs (def text-input          (rx-adapt react-native "TextInput")))
#?(:cljs (def modal               (when-not (= sys/os "web")
                                    (rx-adapt react-native "Modal"))))
#?(:cljs (def scroll-view         (rx-adapt react-native "ScrollView")))
#?(:cljs (def list-view           (rx-adapt react-native "ListView"  )))
...
#?(:cljs (def list-view-data-source (err/ignore (-> react-native .-ListView .-DataSource))))
