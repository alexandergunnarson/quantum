...
(defmacro definline [sym arglist & body]
  (let [body-f
          (->> body (cons 'do)
               (macros/unquote-replacement (ns/context)))]
    (identity ;oeval
      (quote+
  ...
  (condfc component
    ...
    vector?
      (let [[tag props & elems] component
            elems-parsed
              (->> elems (map (fn [elem] (react-parse*- (quote+ (react-parse*- ~elem))))))]
        (concat
          (quote+ (.createElement React ~tag (Object. ~props)))
          elems-parsed))))

; [:div {} ([:div {:prop 1}])]
(defmacro react-parse [component]
  (println "IN REACT-PARSE WITH:" component)
  (oeval (react-parse*- (list 'react-parse*- component))))
